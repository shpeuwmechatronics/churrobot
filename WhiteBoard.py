...
class DrawInput(Widget):
    ...
    def on_touch_down(self, touch):
        if self._in_draw_area(touch.x, touch.y):
            with self.canvas:
                Color(1, 0, 0, 1)
                if self.last_point and self._is_close(self.last_point, (touch.x, touch.y)):
                    Line(points=[*self.last_point, touch.x, touch.y])

                touch.ud["line"] = Line(points=(touch.x, touch.y))
                self.last_point = (touch.x, touch.y)
    ...
    def _is_close(self, p1, p2, threshold=30):
        return hypot(p1[0] - p2[0], p1[1] - p2[1]) <= threshold
